-- First, let's check what statuses are allowed
-- Run this first to see the constraint:
SELECT pg_get_constraintdef(oid) 
FROM pg_constraint 
WHERE conname = 'leads_status_check';

-- If the constraint doesn't allow 'completed', 'quoted', 'lost', let's fix it:
ALTER TABLE leads DROP CONSTRAINT IF EXISTS leads_status_check;
ALTER TABLE leads ADD CONSTRAINT leads_status_check 
  CHECK (status IN ('new', 'contacted', 'quoted', 'completed', 'lost'));

-- Disable RLS
ALTER TABLE leads DISABLE ROW LEVEL SECURITY;

-- Now insert demo leads for sgandhari06@gmail.com
